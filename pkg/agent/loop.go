...
func (al *AgentLoop) buildCommandsRuntime(agent *AgentInstance, opts *processOptions) *commands.Runtime {
	...
	if agent != nil {
		...
		rt.Session = al.buildSessionOps(agent, opts)
	}
	return rt
}
...
func (al *AgentLoop) buildSessionOps(agent *AgentInstance, opts *processOptions) *commands.SessionOps {
	...
	return &commands.SessionOps{
		GetActiveKey: func(senderID string) (string, bool) {
			override := al.state.GetSessionOverride(senderID)
			if override != "" {
				return override, true
			}
			if opts != nil {
				return opts.SessionKey, false
			}
			return "", false
		},
		...
		New: func(senderID, label string) (string, error) {
			autoKey := ""
			if opts != nil {
				autoKey = opts.SessionKey
			}
			newKey := fmt.Sprintf("%s:named:%s", autoKey, label)
			agent.Sessions.GetOrCreate(newKey)
			if err := al.state.SetSessionOverride(senderID, newKey); err != nil {
				return "", err
			}
			return newKey, nil
		},
		...
		Delete: func(senderID, key string) (bool, error) {
			...
			if !wasActive && opts != nil {
				wasActive = key == opts.SessionKey
			}
			if err := agent.Sessions.Delete(key); err != nil {
				return false, err
			}
			if wasActive {
				_ = al.state.ClearSessionOverride(senderID)
			}
			return wasActive, nil
		},
		Reset: func(senderID string) error {
			return al.state.ClearSessionOverride(senderID)
		},
	}
}
